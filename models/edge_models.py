...
class HeatEdge(nn.Module):
    ...
    def forward(self, image_feats, feat_mask, corner_outputs, edge_coords, edge_masks, gt_values, corner_nums,
        ...
        # image_feats -- features, corner_outputs: pixel_features--positional encoder edge_coords: f_coords
        srcs = []
        masks = []
        all_pos = []
        ...
        '''
        edge_feats: edge_coords --- positional encoding --- edge_features --- mlp ---edge_input
        all_pos: positional encoding for each features map
        srcs: CNN_features --- mlp --- new features 把每层金字塔的通道数变成一样（256）
        '''
...
class EdgeTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed, reference_points, labels, key_padding_mask, corner_nums,
        ...
        """
        srcs: features
        pos_embeds: positional encoding for each 2D features map
        query_embeds: edge_input
        reference_points: edge_center
        """
        src_flatten = []
        mask_flatten = []
        ...
        memory = self.encoder(src_flatten, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten,
                              mask_flatten)
        # memory: feature map output from transformer encoding
        ...
        hs_per_edge, _ = self.per_edge_decoder(tgt, reference_points, memory,
                                               ...
                                               mask_flatten)
        # per_edge_decoder: edge Image Feature Fusion
        # f_coord: query_embed
        # f: hs_per_edge
