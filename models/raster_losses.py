...
from utils.bf_utils import get_union_box, rasterize_instances, POLY_LOSS_REGISTRY
import cv2
...
class MaskRasterizationLoss(nn.Module):
    def __init__(self, cfg):
        ...
        #                      torch.from_numpy(np.array([64, 64, 64, 64, 64, 64, 64, 64]).reshape(-1, 2)))
    ...
    def forward(self, preds, targets, target_len, lid=0):
        ...
        for i in range(targets.shape[0]):
            ...
            tgt_poly_single = torch.tensor(np.array(targets[i])).unsqueeze(0).cuda()
            pred_poly_single = torch.tensor(np.array(preds[i])).unsqueeze(0).cuda()
            # inputs:
            # tgt_poly_single: b \times n \times 2 --- 表示一个房间的几个coordinate
            tgt_mask = self.gt_rasterizer(tgt_poly_single * float(resolution[1].item()), resolution[1].item(),
                                          resolution[0].item(), 1.0)
            tgt_mask = (tgt_mask + 1) / 2

            vis_mask = tgt_mask.squeeze().detach().cpu().numpy().astype(np.float32)
            vis_mask = vis_mask * 255.0
            cv2.imshow('vis_mask', vis_mask)
            cv2.waitKey(0)

            pred_mask = self.pred_rasterizer(pred_poly_single * float(resolution[1].item()), resolution[1].item(),
                                             resolution[0].item(), 1.0)
...
class MaskRasterizationCost(nn.Module):
    ...
    def forward(self, preds, targets, target_len, lid=0):
        ...
        for i in range(targets.shape[0]):
            ...
            tgt_mask = self.gt_rasterizer(tgt_poly_single * float(resolution[1].item()), resolution[1].item(),
                                          resolution[0].item(), 1.0)
            pred_masks = self.pred_rasterizer(pred_poly_all * float(resolution[1].item()), resolution[1].item(),
                                              resolution[0].item(), 1.0)
...
if __name__ == '__main__':
    raster_loss = MaskRasterizationLoss(None)
    # load data
    vertex_name = os.path.join('./data/coco_vertex', '00000' + '.npz')
    ...
    floorplan_vertex_index = vertex_dict['floorplan_vertex_index']
    floorplan_vertex_coordinate = vertex_dict['floorplan_vertex_coordinate']
    print('1')
    # compute loss
    loss = raster_loss(floorplan_vertex_coordinate, floorplan_vertex_coordinate, 0)
    print(loss)