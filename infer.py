...
def main(dataset, ckpt_path, image_size, viz_base, save_base, infer_times):
    ...
    if dataset == 'outdoor':
        ...
        # test_dataset = S3DFloorplanDataset(data_path, phase='test', rand_aug=True, inference=True)
    else:
...
def get_results(image, annot, backbone, corner_model, edge_model, pixels, pixel_features,
    ...
    '''
    pixel: pixel_coord (256 * 256 * 2)
    pixel_features: positional encoding for each pixel (256 * 256 * 128) (discrete)
    image: input images
    '''
    ...
    # predict corners --- corner detection
    c_outputs = preds_s1
    ...
    '''
    pred_corners: corners: n * 2 (points)
    edge_coords: [corner_pt1, corner_pt2]: edge_point (n * 2 * 2) --- pair_coords
    '''
...
def get_args_parser():
    ...
    parser.add_argument('--dataset', default='s3d_floorplan',
                        help='the dataset for experiments, outdoor/s3d_floorplan')
    parser.add_argument('--checkpoint_path', default='./checkpoints/ckpts_heat_s3d_256/checkpoint.pth',
