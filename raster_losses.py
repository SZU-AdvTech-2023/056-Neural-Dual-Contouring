...
class MaskRasterizationLoss(nn.Module):
    ...
    def forward(self, preds, targets, target_len, lid=0):
        # inputs: n_rooms \times x
        # compute loss: 是计算所有的还是计算预测为True的？
        ...
        for i in range(targets.shape[0]):
            ...
            tgt_poly_single = targets[i, :target_len[i] * 2].view(-1, 2).unsqueeze(0)
            pred_poly_single = preds[i, :target_len[i] * 2].view(-1, 2).unsqueeze(0)
            ...
            # vis_mask = pred_mask.squeeze().detach().cpu().numpy().astype(np.float32)
...
if __name__ == '__main__':
    ...
    # compute loss
    loss = raster_loss(floorplan_vertex_coordinate, floorplan_vertex_coordinate)
    print(loss)