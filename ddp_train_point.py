...
def get_args_parser():
    ...
    parser.add_argument('--lr_drop', default=[200], type=list)
...
def ddp_trainer(rank, world_size, cfg):
    ...
    edge_loss = nn.CrossEntropyLoss(weight=torch.tensor([1.0, 4.0]).to(device), reduction='mean')
    ...
    for epoch in range(start, cfg.epochs):
        ...
        for batch in train_data_loader:
            ...
            label = batch.get("labels").to(device)
