...
def main(args):
    ...
    exp_dir = os.path.join('/mnt/d/sdb/honghao/heat/experiments', 'implicit_field_decoder_layer_1_w_position')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in tqdm(train_data_loader):
            ...
            # loss = total_loss * 2 + loss_ce
            # sum_cls_loss += loss_ce
            # sum_coord_loss += total_loss
            # # sum_gifs_loss += loss_gifs
            # # print(loss.item())
            gifs = model(inputs, pixel_features, coords)
            loss = F.binary_cross_entropy_with_logits(gifs, flags)
            loss.backward()
            if max_norm > 0:
        ...
        for val_batch in val_data_loader:
            ...
            with torch.no_grad():
                ...
                gifs = model(inputs, pixel_features, p)
